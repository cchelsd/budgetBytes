...
router.get('/:userLogID', async (req, res) => {
  try {
    ...
    const user = await database.readUser(userLogID);
    if (user) {
...
router.put('/:userLogID', async (req, res) => {
  try {
    ...
    const rowsAffected = await database.updateUser(userLogID, updatedUser);
...
router.delete('/:userLogID', async (req, res) => {
  try {
    ...
    const rowsAffected = await database.deleteUser(userLogID);
